...
function LoginphoneNO() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const response = await fetch("http://localhost:5000/login/phoneno", {
                method: 'POST',
                headers: {
        ...
        if (rememberMe) {
            ...
            localStorage.setItem("rememberedphoneNO", phoneNO);
            ...
            localStorage.removeItem("rememberedphoneNO");
    ...
    useState(() => {
        const rememberedphoneNO = localStorage.getItem("rememberedphoneNO");
        const rememberedPassword = localStorage.getItem("rememberedPassword");
        const expiryDate = localStorage.getItem("expiryDate");

        if (rememberedphoneNO && rememberedPassword && expiryDate) {
            const expiry = new Date(expiryDate);
            if (expiry > new Date()) {
                setphoneNO(rememberedphoneNO);
                    ...
                    <div className="main-container w-full max-w-3xl">
                        <div className="main-box bg-black p-16  rounded-md">
                            ...
                            <ul className="social-login grid grid-cols-1 px-40 gap-y-4">
                                ...
                                <li>
                                    <button className="social-btn border phoneNO-social bg-black text-white flex items-center justify-center w-full py-2 rounded-3xl">
                                        <Link className="phoneNO-btn" to="/login">Continue with email</Link>
                                    </button>
                                </li>
                            </ul>
                            ...
                            <div className="signup px-40">
                                <div className="form">
                                    <div className="fieldset mb-6">
                                        <div className="label">
                                            <div className="label">
                                                <label className="username text-sm">phoneNO:</label>
                                            </div>
                                            ...
                                            <div className="label">
                                                ...
                                                <div className="input-password">
                                                    <input
                                                        ...
                                                        title={passwordError ? "Please enter a valid password" : ""}
                                                        required
                                                    />
                                                </div>
                                            </div>
